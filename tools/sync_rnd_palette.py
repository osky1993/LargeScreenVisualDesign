#!/usr/bin/env python3
"""把研发管理专题的公共色板同步进 `screens/rnd-ipd/` 各页（幂等，可反复跑）。

用法：
    python3 tools/sync_rnd_palette.py            # 按色源 JSON 重写各页色板
    python3 tools/sync_rnd_palette.py --remove   # 清空两个块的内容（标记保留）

本脚本是 tools/sync_mfg_palette.py 的研发专题副本，机制完全一致，仅换了
目录、色源与标记串（mfgpalette → rndpalette，--mfg-* → --rnd-*）。

要点：
- 色板单一源是 `shared/palette-rnd.json`：`tokens` 是 token → 色值，
  `_series` 是 ECharts 系列色的 token 名顺序。改色只改这一份，跑本脚本整套页面同步。
- 每页有两处落点，各由一对标记包裹，重跑时**整段替换**而非追加：
    CSS 侧 `:root{}` 内 —— `/* rndpalette:begin … */ … /* rndpalette:end */`，写 `--rnd-*` 变量；
    JS  侧主 `<script>` 内 —— `/* rndpalette-js:begin … */ … /* rndpalette-js:end */`，写 `const PAL`。
...
PALETTE_JSON = os.path.join(ROOT, "shared", "palette-rnd.json")

CSS_BEGIN = "  /* rndpalette:begin —— 由 tools/sync_rnd_palette.py 注入，勿手改 */"
CSS_END = "  /* rndpalette:end */"
JS_BEGIN = "  /* rndpalette-js:begin —— 由 tools/sync_rnd_palette.py 注入，勿手改 */"
JS_END = "  /* rndpalette-js:end */"

# 标记宽松匹配：认 `rndpalette:begin` 这个词即可，注释尾巴的说明文字改了也照样能定位；
# 每次写回都落成上面的规范形态，所以第二次跑必然零改动。
CSS_BLOCK_RE = re.compile(r"[ \t]*/\* *rndpalette:begin.*?/\* *rndpalette:end *\*/", re.S)
JS_BLOCK_RE = re.compile(r"[ \t]*/\* *rndpalette-js:begin.*?/\* *rndpalette-js:end *\*/", re.S)

# 合法 JS 标识符：不是的（如 blue-deep）在对象字面量里必须加引号
...
def css_body(tokens):
    """CSS 侧块体：`--rnd-<token>: <色值>;`，顺序与 JSON 键序一致，便于人工比对。"""
...
def main():
    ...
    for name in names:
        ...
        if not ok_css:
            print("警告：%s 找不到 rndpalette 标记（CSS 侧），已跳过该块" % name)
        if not ok_js:
            print("警告：%s 找不到 rndpalette-js 标记（JS 侧），已跳过该块" % name)
        if out != src:
