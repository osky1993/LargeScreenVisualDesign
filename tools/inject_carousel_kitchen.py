...
用法：
    python3 tools/inject_carousel_kitchen.py            # 注入 / 更新全部 12 页
...
# 厨电制造专题 21 块屏所在目录（页面互链是同目录相对路径，移动目录时只改这里）
PAGES_DIR = os.path.join(ROOT, "screens", "kitchen-appliance")

# 轮播页序 = 采购→制造→品质→仓储→物流→零售→电商→营销→售后→会员→健康 的业务链路顺序，
# 门户页「经营驾驶舱」与 9 个二级子页都不进轮播，勿随意调整
PAGES = [
...
(() => {{
  ...
  /* 轮播页序 = 采购→制造→品质→仓储→物流→零售→电商→营销→售后→会员→健康 业务链路 */
  const PAGES = {pages};
