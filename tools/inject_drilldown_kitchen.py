...
# ---- 扩展批新增的 5 组对象名表 ----------------------------------------------
# 8 个在研项目（研发与新品孵化 → 单项目研发详情）。
# 父页「在研项目明细」表行里写的是**简称**（<b>净享X9</b>），子页 HASH_ALIAS 同时收全名与简称
# （norm = 去空白 + 转大写），故 names 给全名（title 与 hash 更可读）、match 给行上的简称。
RD_PROJECTS = ["净享 X9 洗碗机", "风语 K10 烟机", "悦烤 S7 蒸烤", "净澈 W5 净水",
               "星焰 P11 集成灶", "炽焰 G8 灶具", "静音风机平台", "智控中枢平台"]
RD_SHORT = ["净享X9", "风语K10", "悦烤S7", "净澈W5",
            "星焰P11", "炽焰G8", "静音平台", "智控平台"]

# 18 条产线（设备与工艺参数 → 单产线设备详情）：两页的 PLINES 定义逐字段同源，
# 「设备关联网络」图第三层的节点名就是这里的产线名；子页 HASH_ALIAS 认产线名与编号（L2-03），
# 这里统一给产线名（图上点到什么就传什么，不必再做 match 映射）。
PLINES = ["烟机装配一线", "烟机装配二线", "烟机装配三线", "烟机总装四线",
          "灶具装配一线", "灶具装配二线", "灶具装配三线", "灶具总装四线",
          "洗碗装配一线", "洗碗装配二线", "洗碗总装三线",
          "蒸烤装配一线", "蒸烤总装二线",
          "集成灶一线", "集成灶二线",
          "净水机一线", "热水机二线", "水机总装三线"]

# 三基地能碳（工厂能碳中心 → 单基地能耗详情）：对象名复用上面的 BASES；
# 能流桑基图的基地节点名带「基地」后缀（杭州临平基地），故画布那组用 match 映射。
BASES_NODE = [b + "基地" for b in BASES]

# 12 家重点供应商（供应链风险图谱 → 供应商风险画像）。
# 子页的 FOCUS 是**算出来**的（5 家单一来源 + 战略采购额前 4 + 非单源高风险前 3，去重后按风险降序），
# 不是手点的名单；这里照抄那 12 个字号 —— 只有它们在子页有画像，别的供应商跳过去会落到默认第一家。
# 父页「高风险供应商」表共 15 家（风险 ≥ 50），其中 8 家在这份名单里，只有这 8 行会被标成可下钻。
FOCUS_SUPS = ["华岱精钢", "恒沣铜业", "博岱电子", "泰沣玻璃", "鼎铖钢构", "祁岚电子",
              "泰铖电子", "晟砚电子", "泰锐精钢", "锐砚钢构", "祁屹玻璃", "昊铠玻璃"]

# 13 个重点出口国（海外出口与全球布局 → 单国家市场详情）：子页 FOCUS_META 的 13 国，
# 父页右栏两张表（海外仓布局 / 各国准入认证进度）的行文本里都写国名，直接当 match 用。
FOCUS_CTRY = ["美国", "德国", "沙特阿拉伯", "越南", "泰国", "阿联酋", "马来西亚",
              "法国", "巴西", "意大利", "印度尼西亚", "土耳其", "南非"]
...
# 扩展批的 5 个子页
SUB_RDPROJ = "大屏样板间-厨电单项目研发详情1920.html"
SUB_PLINE = "大屏样板间-厨电单产线设备详情1920.html"
SUB_ENERGY = "大屏样板间-厨电单基地能耗详情1920.html"
SUB_RISK = "大屏样板间-厨电供应商风险画像1920.html"
SUB_CTRY = "大屏样板间-厨电单国家市场详情1920.html"
...
PAGES = [
    ...
    # ---- 以下 5 组为扩展批（17 专题页 + 13 子页）新增，锚点均实测于页面源码 ----

    # ⑨ 研发与新品孵化：右栏「在研项目明细」8 行，行内写项目简称 → 单项目研发详情。
    #    本页没有任何 *-scope id（扩展批 5 页都没有），故 hint chip 挂不上，
    #    改用 panels 在同一面板头挂直达 chip 兜住可发现性（与①同套路）。
    #    另：里程碑甘特画的是已上市的 6 款新品（净享 X7 那批），与在研 8 项不是同一批对象，
    #    绑上去会跳到对不上的项目，故不做画布下钻。
    {"file": "大屏样板间-厨电研发与新品孵化1920.html", "targets": [
        {"rows": ".yf-tbody li", "names": RD_PROJECTS, "match": RD_SHORT, "sub": SUB_RDPROJ,
         "hint": None, "scope": None},
    ], "panels": [
        {"panelTitle": "在研项目明细", "label": "项目详情", "sub": SUB_RDPROJ},
    ]},

    # ⑩ 设备与工艺参数：页上唯一逐条列出 18 条产线的地方是「设备关联网络」力导图的第三层节点
    #    （.sg-tbody li 是**设备类别**台账，8 类设备当不了产线的 hash 对象），故只做画布下钻；
    #    该图父页自己没绑 click，不会与本页交互打架。面板 chip 挂在页头 subnav 声明的
    #    「设备类别台账」上（节点小、label 默认不显示，chip 是兜底入口）。
    {"file": "大屏样板间-厨电设备与工艺参数1920.html", "charts": [
        {"chartId": "sg-chart-graph", "names": PLINES, "sub": SUB_PLINE},
    ], "panels": [
        {"panelTitle": "设备类别台账", "label": "产线详情", "sub": SUB_PLINE},
    ]},

    # ⑪ 工厂能碳中心：左栏「三基地能碳看板」三行卡（行文本写基地全名）+ 中栏能流桑基
    #    （基地节点名是「杭州临平基地」，用 match 映射回全名；子页 hashIdx 其实也认带后缀写法）。
    {"file": "大屏样板间-厨电工厂能碳中心1920.html", "targets": [
        {"rows": ".nt-base-row", "names": BASES, "sub": SUB_ENERGY,
         "hint": None, "scope": None},
    ], "charts": [
        {"chartId": "nt-chart-sankey", "names": BASES, "match": BASES_NODE, "sub": SUB_ENERGY},
    ], "panels": [
        {"panelTitle": "三基地能碳看板", "label": "能耗详情", "sub": SUB_ENERGY},
    ]},

    # ⑫ 供应链风险图谱：右下「高风险供应商」表行（15 家，只有落在子页 12 家画像里的 8 家会被标记）。
    #    四层风险图谱虽然有供应商节点，但父页自己在那张图上绑了 click（点节点看风险传导影响面），
    #    再挂一层跳转会把本页最核心的交互抢掉，故**不做**画布下钻；
    #    另外 4 家战略供应商（风险分低、不进高风险表）走面板 chip 进子页后用页内切换器选。
    {"file": "大屏样板间-厨电供应链风险图谱1920.html", "targets": [
        {"rows": ".fx-rbody li", "names": FOCUS_SUPS, "sub": SUB_RISK,
         "hint": None, "scope": None},
    ], "panels": [
        {"panelTitle": "高风险供应商", "label": "风险画像", "sub": SUB_RISK},
    ]},

    # ⑬ 海外出口与全球布局：右栏两张表都是 .hw-tbody li，一条选择器同时吃下
    #    「海外仓布局」（马来西亚·巴生 / 德国·汉堡…）与「各国准入认证进度」（SASO 沙特阿拉伯…），
    #    命中 13 个重点出口国即可下钻；波兰仓、欧盟/海湾等非单一国家的行不在名单里，保持不可点。
    #    地球是 3D 自转球（scatter3D 打的是海外仓与港口，不是国家），点它体验差且对象口径不符，不绑。
    {"file": "大屏样板间-厨电海外出口与全球布局1920.html", "targets": [
        {"rows": ".hw-tbody li", "names": FOCUS_CTRY, "sub": SUB_CTRY,
         "hint": None, "scope": None},
    ], "panels": [
        {"panelTitle": "海外仓布局", "label": "市场详情", "sub": SUB_CTRY},
    ]},
]
