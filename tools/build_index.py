...
CONTENT_DIRS = [
    ...
    "screens/kitchen-appliance",  # 厨电制造专题样板间
    "screens/rnd-ipd",      # 研发管理专题样板间
    "components",           # 大屏组件与脚手架
    "palettes",             # 配色陈列
    "studies",              # 选型/效果对比
]
...
CATEGORY_RULES = [
    ...
    ("大屏样板间-公共资金", "公共资金专题"),
    # 同理：厨电制造专题文件名也同时含「样板间」与「地图/柱状图」等关键词，须先于通用样板间规则匹配。
    ("大屏样板间-厨电", "厨电制造专题"),
    # 同理：研发管理专题（前缀锚定「大屏样板间-研发」，不会误吞「大屏样板间-厨电研发与新品孵化」）。
    ("大屏样板间-研发", "研发管理专题"),
...
FRONT_CATEGORIES = [
    ...
    "区间目标", "弦图", "路径图", "数据集变换", "大屏组件", "公共资金专题", "厨电制造专题", "研发管理专题", "样板间", "配色方案", "其他",
]
FRONT_BADGE_COLORS = {
    ...
    "厨电制造专题": "#35e0d0",  # 工业青，与该系列页面主色一致
    "研发管理专题": "#5c9dff",  # 星蓝，与该系列页面主色一致
}
...
FRONT_FAMILIES = [
    ...
    "公共资金专题样板间", "厨电制造专题样板间", "研发管理专题样板间", "样板间", "大屏组件", "其他",
]
FAMILY_OF = {
    ...
    "厨电制造专题": "厨电制造专题样板间",  # 独立成组，不与通用样板间混排
    "研发管理专题": "研发管理专题样板间",  # 独立成组，不与通用样板间混排
    "其他": "其他",
}
...
def scan_file(path: Path) -> dict:
    ...
    if "配色方案-21套" in stem:  # 普通 HTML，方案数标 21 套配色
        count = 21
    if cat in ("样板间", "公共资金专题", "厨电制造专题", "研发管理专题"):  # 整屏样板间为 1 个成品大屏，文件名数字是分辨率而非方案数
        count = 1
